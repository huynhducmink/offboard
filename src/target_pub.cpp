...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        target.pose.position.y = rand() % (max - min + 1) + min;
        target.pose.position.z = 0;
        check.data = sqrt((max - current.pose.position.x)*(max - current.pose.position.x)
        + (max - current.pose.position.y)*(max - current.pose.position.y));
